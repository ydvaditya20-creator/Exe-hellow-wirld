...
SPREADSHEET_ID = "1qAG5pvjpj0iLEi0RPjEMC_kV01bceCY7sBNys7-xitE";
...
var SPREADSHEET_ID = "1qAG5pvjpj0iLEi0RPjEMC_kV01bceCY7sBNys7-xitE";
...
function setupDatabase() {
  ...
  // 🌟 NAYA HISSA: Teesra Tab banana HTML Vouchers save karne ke liye
  var voucherSaveSheet = ss.getSheetByName("SavedVouchers");
  if (!voucherSaveSheet) {
    voucherSaveSheet = ss.insertSheet("SavedVouchers");
    // Column Headers सेट करना
    voucherSaveSheet.getRange(1, 1, 1, 4).setValues([["DATE", "VOUCHER NO.", "VOUCHER TYPE", "HTML TEXT LAYOUT"]]);
    voucherSaveSheet.getRange(1, 1, 1, 4).setFontWeight("bold").setBackground("#d9ead3");
  }
...
function doPost(e) {
  try {
    ...
    var sheetName = params.category;     // "Particulars", "Heads" ya "HtmlVoucher"
    
    // 🌟 NAYA HISSA: Agar category "HtmlVoucher" hai toh teesre tab mein save karein
    if (sheetName === "HtmlVoucher") {
      var htmlSheet = ss.getSheetByName("SavedVouchers");
      if (!htmlSheet) {
        htmlSheet = ss.insertSheet("SavedVouchers");
        htmlSheet.getRange(1, 1, 1, 4).setValues([["DATE", "VOUCHER NO.", "VOUCHER TYPE", "HTML TEXT LAYOUT"]]);
      }
      
      // Nayi row append karna [Date, VNo, VType, HTML Content]
      htmlSheet.appendRow([
        params.date,
        params.voucherNo,
        params.voucherType,
      ...
      return ContentService.createTextOutput(JSON.stringify({status: "success", message: "HTML Voucher Saved Successfully"}))
        .setMimeType(ContentService.MimeType.JSON);
    }
    
    // --- AAPKA PURANA LOGIC (Particulars & Heads ke liye) ---
    var voucherType = params.voucherType; // e.g., "cash_receipt"
    var newValue = params.newValue;
